...
- `WasmExecutionBackend.compile()` stdExtensions: ✅ 実装済み
- `WasmExecutionBackend.compile()` optPasses: ❌ 未実装
- `WasmExecutionBackend.runAsync()` stdExtensions (`WasmWhitespaceVM` コンストラクタ): ✅ 実装済み
- `WasmExecutionBackend.run()` 第4引数 stdExtensions: ✅ 実装済み
- `useNospaceExecution.handleRun()` stdExtensions: ✅ 実装済み
- `useNospaceExecution` optPasses (ExecutionOptions): ❌ 未実装
- `WasmExecutionBackend.spec.ts` optPasses テスト: ❌ 未実装
