...
- [x] WASM flavor で Run(Direct) タブが表示される
- [x] Run(Direct) タブから実行ボタンを押すと WasmNospaceVM で実行される
- [x] stdout が出力パネルに表示される
- [x] エラー時に stderr が出力される
- [x] コンパイルエラー時にエディタアノテーションが表示される
- [x] ignoreDebug オプションが反映される
- [x] optPasses オプションが反映される
- [x] kill ボタンで実行が中断される
- [x] 実行制限（maxTotalSteps）で停止する
- [x] 既存の Compile / Run モードが壊れていない
